...
func (suite *SyncEntityTestSuite) TestInsertSyncEntity() {
	...
	entity2 := entity1
	entity2.ID = "id2"
	_, err := suite.dynamo.InsertSyncEntity(context.Background(), &entity1)
	suite.Require().NoError(err, "InsertSyncEntity should succeed")
	_, err = suite.dynamo.InsertSyncEntity(context.Background(), &entity2)
	...
	_, err = suite.dynamo.InsertSyncEntity(context.Background(), &entity3)
	suite.Require().NoError(err, "InsertSyncEntity should succeed")
	...
	_, err = suite.dynamo.InsertSyncEntity(context.Background(), &entity4)
	...
	conflict, err := suite.dynamo.InsertSyncEntity(context.Background(), &entity4Copy)
	...
	entity5.ID = "id5"
	_, err = suite.dynamo.InsertSyncEntity(context.Background(), &entity5)
...
func (suite *SyncEntityTestSuite) TestHasServerDefinedUniqueTag() {
	...
	err := suite.dynamo.InsertSyncEntitiesWithServerTags(context.Background(), entities)
	suite.Require().NoError(err, "Insert sync entities should succeed")

	hasTag, err := suite.dynamo.HasServerDefinedUniqueTag(context.Background(), "client1", "tag1")
	suite.Require().NoError(err, "HasServerDefinedUniqueTag should succeed")
	suite.True(hasTag)

	hasTag, err = suite.dynamo.HasServerDefinedUniqueTag(context.Background(), "client1", "tag2")
	suite.Require().NoError(err, "HasServerDefinedUniqueTag should succeed")
	suite.False(hasTag)

	hasTag, err = suite.dynamo.HasServerDefinedUniqueTag(context.Background(), "client2", "tag1")
	suite.Require().NoError(err, "HasServerDefinedUniqueTag should succeed")
	suite.False(hasTag)

	hasTag, err = suite.dynamo.HasServerDefinedUniqueTag(context.Background(), "client2", "tag2")
	suite.Require().NoError(err, "HasServerDefinedUniqueTag should succeed")
	suite.True(hasTag)
}
...
func (suite *SyncEntityTestSuite) TestHasItem() {
	...
	hasTag, err := suite.dynamo.HasItem(context.Background(), "client1", "id1")
	suite.Require().NoError(err, "HasItem should succeed")
	suite.True(hasTag)

	hasTag, err = suite.dynamo.HasItem(context.Background(), "client2", "id2")
	suite.Require().NoError(err, "HasItem should succeed")
	suite.True(hasTag)

	hasTag, err = suite.dynamo.HasItem(context.Background(), "client2", "id3")
	suite.Require().NoError(err, "HasItem should succeed")
	suite.False(hasTag)

	hasTag, err = suite.dynamo.HasItem(context.Background(), "client3", "id2")
	suite.Require().NoError(err, "HasItem should succeed")
	suite.False(hasTag)
}
...
func (suite *SyncEntityTestSuite) TestInsertSyncEntitiesWithServerTags() {
	...
	suite.Require().Error(
		suite.dynamo.InsertSyncEntitiesWithServerTags(context.Background(), entities),
		"Insert with same ClientID and server tag would fail")
...
func (suite *SyncEntityTestSuite) TestUpdateSyncEntity_Basic() {
	...
	conflict, deleted, err := suite.dynamo.UpdateSyncEntity(context.Background(), &updateEntity1, *entity1.Version)
	...
	conflict, deleted, err = suite.dynamo.UpdateSyncEntity(context.Background(), &updateEntity2, *entity2.Version)
	suite.Require().NoError(err, "UpdateSyncEntity should succeed")
	suite.False(conflict, "Successful update should not have conflict")
	...
	conflict, deleted, err = suite.dynamo.UpdateSyncEntity(context.Background(), &updateEntity3, *entity3.Version)
	suite.Require().NoError(err, "UpdateSyncEntity should succeed")
	suite.False(conflict, "Successful update should not have conflict")
	suite.False(deleted, "Non-delete operation should return false")
...
func (suite *SyncEntityTestSuite) TestUpdateSyncEntity_HistoryType() {
	...
	conflict, err := suite.dynamo.InsertSyncEntity(context.Background(), &entity1)
	suite.Require().NoError(err, "InsertSyncEntity should succeed")
	...
	conflict, deleted, err := suite.dynamo.UpdateSyncEntity(context.Background(), &updateEntity1, 1)
	suite.Require().NoError(err, "UpdateSyncEntity should succeed")
	suite.False(conflict, "Successful update should not have conflict")
	suite.False(deleted, "Non-delete operation should return false")
	...
	conflict, deleted, err = suite.dynamo.UpdateSyncEntity(context.Background(), &updateEntity2, 1)
	...
	conflict, deleted, err = suite.dynamo.UpdateSyncEntity(context.Background(), &updateEntity3, 1)
	suite.Require().NoError(err, "UpdateSyncEntity should succeed")
	suite.False(conflict, "Successful update should not have conflict")
	suite.True(deleted, "Delete operation should return true")
...
func (suite *SyncEntityTestSuite) TestUpdateSyncEntity_ReuseClientTag() {
	...
	conflict, deleted, err = suite.dynamo.UpdateSyncEntity(context.Background(), &updateEntity1, 23456789)
	suite.Require().NoError(err, "UpdateSyncEntity should succeed")
	suite.False(conflict, "Successful update should not have conflict")
	suite.True(deleted, "Delete operation should return true")
	...
	entity2 := entity1
	entity2.ID = "id2"
	conflict, err = suite.dynamo.InsertSyncEntity(context.Background(), &entity2)
	suite.Require().NoError(err, "InsertSyncEntity should succeed")
	suite.False(conflict, "Successful insert should not have conflict")
...
func (suite *SyncEntityTestSuite) TestDisableSyncChain() {
	clientID := "client1"
	id := "disabled_chain"
	err := suite.dynamo.DisableSyncChain(context.Background(), clientID)
...
func (suite *SyncEntityTestSuite) TestIsSyncChainDisabled() {
	...
	disabled, err := suite.dynamo.IsSyncChainDisabled(context.Background(), clientID)
...
// page and deletes every item for the target chain, and that items belonging
// to other chains are not touched.
//
...
func (suite *SyncEntityTestSuite) TestClearServerDataPagination() {
	...
	insert(clientID, itemCount, blob)
	// Keep other-chain items small — ScanSyncEntities doesn't paginate, so a
	// large blob here could push the post-clear scan past the 1MB page cap
	// and cause flaky Len assertions.
	insert(otherClientID, otherClientItemCount, nil)
	...
	for _, item := range cleared {
		suite.Equal(clientID, item.ClientID, "ClearServerData must only return items for the target chain")
	}

	remaining, err := datastoretest.ScanSyncEntities(suite.dynamo)
	suite.Require().NoError(err, "ScanSyncEntities should succeed")
	suite.Lenf(remaining, otherClientItemCount,
		"items belonging to other chains must not be deleted: expected %d remaining, got %d",
		otherClientItemCount, len(remaining))
	for _, item := range remaining {
		suite.Equal(otherClientID, item.ClientID, "only the other chain's items should remain")
	}
}
