...
func (_d DatastoreWithPrometheus) GetClientItemCount(ctx context.Context, clientID string) (cp1 *ClientItemCounts, err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
...
func (_d DatastoreWithPrometheus) GetUpdatesForType(ctx context.Context, dataType int, clientToken int64, fetchFolders bool, clientID string, maxSize int) (b1 bool, sa1 []SyncEntity, err error) {
	_since := time.Now()
	defer func() {
		result := "ok"
		if err != nil {
